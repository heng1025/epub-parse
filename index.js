...
function parseEpubBook(uri) {
  ...
  return fetchContainerXml(uri)
    ...
    .then(rootXml => {
      ...
      const tocManifestId = manifest.items.find(id => id === rootXml.package.spine.toc);
 ...
 * @returns {Promise}
 */
function loadEpubChapter(rootURL, packageDirectory, spine, manifest, chapterCount) {
  const fileReg = /('|")[^'|"]*\.(jpg|png|bmp|jpeg|gif|mp3|wma|ogg|3gp|mp4|avi|wmv)\1/ig;
  const srcReg = /src=(?=('|")[^'|"]*\.(jpg|png|gif|bmg|jpeg)\1)/ig;
  const contentLinkReg = /href=["'](.*?)\.(x?)html(#?)(.*?)['"]/ig;
  const splitReg1 = /(\>)([^\>\<]+)(\<)/g;
  ...
  return fetchChaterXml(rootURL, chapterPath, packageDirectory).then(chapterText => {
